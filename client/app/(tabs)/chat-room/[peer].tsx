...
export default function DirectChatScreen() {
  const { peer: peerId } = useLocalSearchParams<{ peer: string }>(); // Matches [peer].tsx filename
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const resp = await (await import('@/lib/api')).api.get<any[]>(`/chat/messages/${encodeURIComponent(me.username)}/${encodeURIComponent(peerId)}`);
        const msgs = resp.data.map((m: any) => ({
          id: m._id || m.id,
          from: m.from,
          content: m.content ?? m.fileUrl ?? '',
          timestamp: m.timestamp ?? m.created_at ?? new Date().toISOString(),
          status: m.from === me.username ? 'sent' : 'sent',
        } as Message));
        setMessages(msgs);
    ...
    const onMessage = (msg: Message) => {
      if (msg.from === peerId || msg.from === me.username) {
        setMessages((prev) => {
          // IMPORTANT: Prevent duplicate messages!
  ...
  const sendMessage = useCallback(() => {
    ...
    const newMsg: Message = {
      id: tempId,
      from: me.username, // Ensures Left/Right bubble logic works accurately
      content: draft.trim(),
  ...
  const renderMessage = ({ item }: { item: Message }) => {
    // If it's from 'me', it goes on the right. If from 'peer', it goes on the left.
