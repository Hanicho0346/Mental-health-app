...
export async function initiateBooking(params: {
  ...
  const [psychiatrist, user, existing] = await Promise.all([
    User.findById(psychiatristId).lean(),
  ...
  if (existing)
    throw new AppError(409, 'You already have an active booking with this psychiatrist');
...
export async function verifyAndCompleteBooking(tx_ref: string) {
  ...
  if (!success) {
    await Booking.updateOne({ _id: booking._id }, { $set: { payment_status: 'failed' } });
  ...
  const updated = await Booking.findOneAndUpdate(
    ...
    { $set: { payment_status: 'paid', booking_status: 'confirmed', already_processed: true } },
    { new: true }
  ).lean();
  ...
  const psychiatristId = updated.psychiatrist_id as mongoose.Types.ObjectId;
  const userId         = updated.user_id as mongoose.Types.ObjectId;
  ...
  const walletWrites: Promise<unknown>[] = [
...
export async function getWalletForUser(userId: string) {
  ...
  const [user, transactions] = await Promise.all([
    User.findById(uid).select('wallet_balance').lean(),
    WalletTransaction.find({ user_id: uid })
      .sort({ created_at: -1 })
      .limit(50)
      .lean(),
  ]);
...
export async function listAllBookings(filters?: {
  ...
  const users   = await User.find({ _id: { $in: userIds } }).select('full_name email role').lean();
